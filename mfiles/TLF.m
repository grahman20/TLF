function P = TLF(Xs,Ys,Xt,Yt,options)
...
%     Cls = knn_model.predict(X(:,n + 1:end)');
    ...
    % Construct kernel
    K = kernel_tlf('rbf',X,sqrt(sum(sum(X .^ 2).^0.5)/(n + m)));
...
%         XX=[Xs_proj;Xt']';
%         Yall=[Ys;Yt];
%         knn_model = fitcknn(XX(:,1:end)',Yall,'NumNeighbors',1);
%         Cls = knn_model.predict(X(:,n + 1:end)');        
%         F = K * Beta;
%         [~,Cls] = max(F,[],2);

        %% Compute accuracy
%         Acc = numel(find(Cls(1:end)==Yt)) / m;
%         Cls = Cls(1:end);
...
function K = kernel_tlf(ker,X,sigma)
    switch ker
        case 'linear'
            K = X' * X;
        case 'rbf'
